...
//   onAddOrder: (snapshot: FacilityDto) => void; // new: called when user clicks '+'
//   open: boolean;
// }
...
//   // local selected ids for SKU and State (so UI is responsive immediately)
//   // const [selectedSkuId, setSelectedSkuId] = useState<string>(
//   //   facility?.skuOpt?.id ?? ""
//   // );
//   const [selectedSkuCode, setSelectedSkuCode] = useState<string>(
//     facility?.skuOpt?.name ?? ""
//   );
...
//     // sync selected sku/state ids locally
//     setSelectedSkuCode(facility?.skuOpt?.id ?? "");
//     setSelectedStateId(facility?.stateOpt?.id ?? "");
//     setLocalArea((facility?.area ?? 0) === 0 ? "" : facility?.area ?? "");

//     // if facility has skuOpt, try to determine parentId from loaded skuDesigns
//     if (facility?.skuOpt?.id && skuDesigns.length > 0) {
//       const sel = skuDesigns.find((s) => s.id === facility.skuOpt?.id);
...
//   const handleAdd = () => {
//     // Build snapshot merging parent `facility` (customer/general) and local order-specific fields
//     const skuSel = skuDesigns.find((s) => s.id === selectedSkuCode);
