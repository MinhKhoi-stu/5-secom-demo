...
const UpdateOrder: React.FC<UpdateOrderProps> = ({
  ...
  const { mutateAsync: updateFacility, isLoading: isUpdating } =
    useUpdateFacility();
  ...
  const selectedLevel1Id = useMemo(() => {
    const f = lvl1Options.find(
      (o: OptionItem) => (o.name ?? "") === selectedLevel1
    );
    return f?.id ?? "";
  }, [lvl1Options, selectedLevel1]);

  const selectedLevel2Id = useMemo(() => {
    const f = lvl2Options.find(
      (o: OptionItem) => (o.name ?? "") === selectedLevel2
    );
    return f?.id ?? "";
  ...
  const filteredLvl2 = useMemo(() => {
    ...
    return lvl2Options.filter(
      (o: OptionItem) => getParentId(o) === selectedLevel1Id
    );
  }, [lvl2Options, selectedLevel1Id]);

  const filteredLvl3 = useMemo(() => {
    if (!selectedLevel2Id) return lvl3Options;
    return lvl3Options.filter(
      (o: OptionItem) => getParentId(o) === selectedLevel2Id
    );
  ...
  // --- UPDATED: use useUpdateFacility to persist changes ---
  const handleSave = async () => {
    try {
      // Build payload similarly như trước, nhưng include id + version (server expects those)
      const payload: any = {
        // required by UpdateFacilityDto
        version: facility?.version ?? 0,
        id: facility?.id,
        ...
        // keep productTypeId/statusId in payload if your backend accepts them
      ...
      if (selectedSkuId) {
        ...
        // fallback: send skuOpt.code if server supports it
        payload.skuOpt = { code };
      }
      ...
      // call mutation
      await updateFacility({ id: facility.id, data: payload });

      // call parent onSaved if provided (parent can do extra refresh or side-effects)
      if (typeof onSaved === "function") {
        ...
        // Fallback: ensure list refresh if parent didn't provide onSaved
        // your useUpdateFacility already invalidates FIND_ALL_FACILITY, but keep fallback
          ...
          <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 2 }}>
            <Button
              ...
              sx={{
                borderRadius: "8px",
                width: "40%",
                backgroundColor: isUpdating ? "#ef9a9a" : "lightsalmon",
                color: "black",
                ":hover": { backgroundColor: "tomato" },
              }}
            >
              {isUpdating ? <CircularProgress size={20} /> : "LƯU CHỈNH SỬA"}
            </Button>
          </Box>
        </Grid>

        {/* Dưới cùng, nút lưu dùng trạng thái isUpdating */}
